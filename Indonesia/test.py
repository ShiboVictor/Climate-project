...
# %%
dataset = pd.read_csv("Indonesia-DroughtPrediction.csv", encoding='UTF-8')
print(dataset)
...
# %%
dataset_vmd = pd.read_excel("CEEMDAN-INDONESIA-CDD.xlsx")
...
def cnn_lstm_model(input_shape, output_dim):
    ...
    x = Dropout(0.2)(x)  # 添加Dropout防止过拟合
    ...
    x = LSTM(128, activation='tanh', return_sequences=True,
             recurrent_dropout=0.1)(x)  # 使用tanh减少梯度消失
...
def weighted_mse_loss(y_true, y_pred):
    """
    加权MSE损失函数，对较大值给予更多权重
    """
    mse = K.square(y_true - y_pred)
    # 对大于均值的值增加权重
    weights = K.cast(y_true > K.mean(y_true), K.floatx()) * 1.5 + 1.0
...
for vmd_num in range(values_vmd_aligned.shape[1]):
    ...
    # 改进的归一化策略 - 使用RobustScaler对异常值更鲁棒
    from sklearn.preprocessing import RobustScaler

    # 对于输入特征使用MinMaxScaler
    m_in = MinMaxScaler()
    ...
    # 对于IMF目标值，先判断是否主要为正值
    if imf.min() >= 0:
        # 如果IMF都是非负的，使用MinMaxScaler到[0,1]
        m_out = MinMaxScaler(feature_range=(0, 1))
    else:
        # 如果IMF有负值，使用标准MinMaxScaler
        m_out = MinMaxScaler()
    ...
    model.fit(
        vp_train, vt_train,
        batch_size=16,  # 减小batch size提高对细节的学习
        epochs=100,  # 增加epochs但使用early stopping
        validation_split=0.15,  # 添加验证集用于监控
        shuffle=True,  # 打乱数据提高泛化能力
...
for ii in range(n_out):
    ...
    if n_out == 1:
        plt.title(
            f"The prediction result of CEEMDAN-CNN-LSTM :\nMAPE: {mape(actual_test[:, ii], pre_test[:, ii]):.2f} %")
    else:
        plt.title(
            f"{ii + 1} step of CEEMDAN-CNN-LSTM prediction\nMAPE: {mape(actual_test[:, ii], pre_test[:, ii]):.2f} %")
