...
# 分离特征和目标变量（最后一列视为 CDD）
features_aligned = values_aligned[:, :-1]   # 原始特征（不包含cdd）
target_aligned   = values_aligned[:, -1]    # 目标变量（cdd）
...
pre_test = m_out.inverse_transform(yhat)  # 反归一化得到预测的 CDD
...
csv_path = "LSTM_classification_results.csv"
