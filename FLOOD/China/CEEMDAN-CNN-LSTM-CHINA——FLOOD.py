...
# %%
dataset = pd.read_csv("CHINA-FloodPrediction.csv", encoding='UTF-8')
print(dataset)
...
# %%
dataset_vmd = pd.read_excel("CEEMDAN-CHINA-RX5DAY.xlsx")
...
# 特征与原始目标
features_aligned = values_aligned[:, :-1]  # 不含 rx5day 的气象因子
target_aligned   = values_aligned[:, -1]   # rx5day
...
# %%
# 逐 IMF 建模（用原始多变量特征 -> 预测该 IMF），预测完再求和重构 rx5day
...
# %%
# 汇总各 IMF 的预测并重构最终预测 rx5day（与原脚本相同做法）
...
# ================== rx5day 三分类（Non-flood / Slight flood / Flood）评估 ==================
import numpy as np
import pandas as pd
...
# 1) 扁平化
y_pred_rx5day = pre_test.reshape(-1)      # 预测 rx5day
y_true_rx5day = actual_test.reshape(-1)   # 真实 rx5day
months = pd.to_datetime(pd.Series(test_months))

# 2) 阈值规则：<50 = Non-flood, 50–100 = Slight flood, ≥100 = Flood
...
print(classification_report(
    ...
    labels=labels_order,          # 显式指定三类，避免缺类时报错
    target_names=target_names,    # 与 labels 顺序一致
    digits=4,
    zero_division=0
))
...
csv_path = "CEEMDAN_CNN_LSTM_Flood_classification_results.csv"
