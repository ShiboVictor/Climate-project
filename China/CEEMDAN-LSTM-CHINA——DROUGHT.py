...
# 使用pandas模块的read_csv函数读取名为"China-DroughtPrediction.csv"的文件。
...
# 使用pandas模块的read_excel函数读取名为"CEEMDAN-CHINA-CDD.xlsx"的文件。
...
# 分离特征和目标变量
features_aligned = values_aligned[:, :-1]  # 原始特征（不包含cdd）
target_aligned = values_aligned[:, -1]  # 原始目标变量（cdd）
...
for vmd_num in range(0, len(values_vmd_aligned[0])):
    ...
    # 训练模型。指定批处理大小为32，训练轮数为50，将0%的数据用作验证集。
...
# ================== CDD 3-class classification (Non/Slight/Drought) ==================
import numpy as np
import pandas as pd
...
# Flatten series
y_pred_cdd = pre_test.reshape(-1)     # predicted CDD
y_true_cdd = actual_test.reshape(-1)  # true CDD
months = pd.to_datetime(pd.Series(test_months))

# Thresholds: <11 Non-drought, 11–15 Slight drought, >=16 Drought
...
print("\n=== CDD classification (thresholds: <11=Non-drought, 11–15=Slight drought, ≥16=Drought) ===")
...
print("\n[Confusion matrix] Row = True, Col = Pred (0=Non-drought, 1=Slight drought, 2=Drought):")
print(cm)
...
# Export per-month classification
out_df = pd.DataFrame({
    "Month": months.values,
    "cdd_true": y_true_cdd,
    "cdd_pred": y_pred_cdd,
...
csv_path = "CEEMDAN_LSTM_classification_results.csv"
